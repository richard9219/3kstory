...
func SetupRoutes(r *gin.Engine, db *gorm.DB, rdb *redis.Client, cfg *config.Config) {
	...
	storyboardService := services.NewStoryboardService(db, videoService, cfg)
	...
	v1 := r.Group("/api/v1")
	{
		...
		authorized.Use(middleware.AuthRequired(cfg))
		{
			...
			projects := authorized.Group("/projects")
			{
				...
				projects.POST("/:id/generate-narration-advanced", narrationHandler.GenerateNarrationAdvanced)
				...
				projects.GET("/:id/video-jobs/:jobID", narrationHandler.GetVideoJob)
				projects.POST("/:id/video-jobs/:jobID/select", narrationHandler.SelectVideoCandidate)
				projects.POST("/:id/video-jobs/:jobID/auto-publish", narrationHandler.AutoPublishWithGate)
				...
				projects.GET("/:id/storyboard-timeline", storyboardHandler.GetTimeline)
				projects.PATCH("/:id/storyboard-shots/:shotID", storyboardHandler.UpdateShot)
				projects.POST("/:id/storyboard-shots/:shotID/generate", storyboardHandler.GenerateShotClip)
				projects.POST("/:id/storyboard-shots/:shotID/regenerate", storyboardHandler.RegenerateShotClip)
				projects.POST("/:id/storyboard-timeline/render", storyboardHandler.RenderDirectorCut)
				projects.GET("/:id/storyboard-timeline/exports/:exportID", storyboardHandler.StreamDirectorCut)
				projects.POST("/:id/storyboard-timeline/exports/:exportID/auto-publish", storyboardHandler.CheckDirectorCutGate)
				projects.POST("/:id/storyboard-timeline/exports/:exportID/publish", storyboardHandler.PublishDirectorCut)
				projects.GET("/:id/storyboard-timeline/publish-history", storyboardHandler.ListDirectorPublishHistory)
				projects.POST("/:id/storyboard-timeline/publish-history/:recordID/retry", storyboardHandler.RetryDirectorPublish)
				projects.GET("/:id/director-templates", storyboardHandler.ListDirectorTemplates)
				projects.POST("/:id/director-templates", storyboardHandler.CreateDirectorTemplate)
				projects.PUT("/:id/director-templates/:templateID", storyboardHandler.UpdateDirectorTemplate)
				projects.DELETE("/:id/director-templates/:templateID", storyboardHandler.DeleteDirectorTemplate)
				projects.POST("/:id/director-agent/auto-strategy", storyboardHandler.AutoDirectorStrategy)
				projects.POST("/:id/director-agent/ab-compare", storyboardHandler.CompareDirectorAB)
			}
		}
	}
}
